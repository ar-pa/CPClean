...
namespace FFT {
	...
	void mult(value *a, value *b, value *c, int len) {
		...
		REP(i, len) a1[i] = a[i];
		REP(i, len) a2[i] = b[i];
		REP(i, N) omega[i] = polar(1.0, 2*PII/N*i);
		...
		REP(i, 2*len) c[i] = (ll) round(z1[i].real()) % mod;
	}
	...
	void mult_mod(int *a, int *b, int *c, int len, int mod) {
		...
		//REP(i, 2*len) c1[i] = ((ll) c1[i] - c0[i] - c2[i] + 2 * mod) % mod;
 
		//REP(i, 2*len) c1[i] %= mod;
		//REP(i, 2*len) c2[i] %= mod;
		REP(i, 2*len) c[i] = (c0[i] + ((long long) c1[i] - c0[i] - c2[i] + 2 * mod << 16) + ((long long) c2[i] << 32)) % mod;
	}
}
#undef REP
